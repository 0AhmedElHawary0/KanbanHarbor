# KanbanHarbor - Clean Architecture, DDD & CQRS

KanbanHarbor applies the principles of Clean Architecture, Domain-Driven Design (DDD), and Command Query Responsibility Segregation (CQRS) to create a robust, scalable, and maintainable multi-tenant project execution platform. It is designed for developers who aim to build complex systems with a clear separation of concerns and a strong alignment between business domain and technology.
...
1. Create Project directly via composer:
    ```bash
     composer create-project shahghasiadil/kanbanharbor
    ```
1. Clone the repository:
    ```bash
    git clone https://github.com/0AhmedElHawary0/KanbanHarbor.git
    ```
...
-   **Application**: Manages the application logic and orchestrates the flow of data between the domain and presentation layers. This layer includes:
    ...
    -   `User`: Contains all user-related business logic, divided into:
        ...
        -   `QueryHandlers`: Handles queries and fetches read data without mutating state.
...
-   **Domain**: The heart of the business logic, defining entities, value objects, and domain events.
    ...
    -   `User`: Domain logic and entities specific to user management, including:
        ...
        -   `ValueObjects`: Immutable domain primitives (for example email/name wrappers) that protect invariants.
...
-   **Infrastructure**: Implements the interfaces defined by the domain layer, dealing with data persistence and external systems.
    ...
    -   `User`: Infrastructure logic specific to user management, including:
        ...
        -   `Persistence`: Persistence mechanisms for user data, including:
            -   `Models`: ORM/database models and persistence mappers.
